...
### Build and Test the Metadata API

This sample also includes a small off-chain metadata API. It stores non-sensitive
campaign labels outside the Compact contract so the app can demonstrate the
boundary between on-chain public state and off-chain product metadata.
...
The metadata API defaults to `http://127.0.0.1:8088`. Set
`VITE_METADATA_API_URL` in `bboard-ui/.env.preprod` or `bboard-ui/.env.preview`
if you run it somewhere else.
